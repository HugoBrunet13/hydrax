...
module.exports = class cezex extends Exchange {
    ...
    async fetchTrades (symbol, limit = undefined, timestamp = undefined, params = {}) {
        let since = undefined;
        await this.loadMarkets ();
        let market = this.market (symbol);
        let request = {
    ...
    async fetchOrders (symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        request['state']='wait';
        let waitOrders = await this.privateGetOrders (this.extend (request, params));

        request['state']='done';
        let doneOrders = await this.privateGetOrders (this.extend (request, params));
        
        request['state']='cancel';
        let cancelOrders = await this.privateGetOrders (this.extend (request, params));

        let allOrders = [];
        if(waitOrders.length>0) {
            for(let i = 0; i<waitOrders.length;++i){
                allOrders.push([waitOrders[i]]);
            }
        }
        if (doneOrders.length>0) {
            for(let i = 0; i<doneOrders.length;++i){
                allOrders.push([doneOrders[i]]);
            }
        }
        if (cancelOrders.length>0) {
            for(let i = 0; i<this.cancelOrders.length;++i){
                allOrders.push([cancelOrders[i]]);
            }  
        }
              
        return this.parseOrders (allOrders, market, since, limit);
    }
